 ...
 * into the topology using @h as parent. The controller's own configuration
 * (set from JSON config or via libnvme_ctrl_* setters) is used for the
 * connection.
...
/**
 * libnvmf_context_set_io_options() - Set I/O queue parameters for context
 * @fctx:		Fabrics context
 * @queue_size:		Number of I/O queue entries (0 = kernel default)
 * @nr_io_queues:	Number of I/O queues (0 = kernel default)
 * @nr_write_queues:	Number of write-only queues (0 = disabled)
 * @nr_poll_queues:	Number of poll queues (0 = disabled)
 *
 * Sets I/O queue sizing parameters for the context.
 *
 * Return: 0 on success, or a negative error code on failure.
 */
int libnvmf_context_set_io_options(struct libnvmf_context *fctx,
		int queue_size, int nr_io_queues,
		int nr_write_queues, int nr_poll_queues);

/**
 * libnvmf_context_set_timeout_options() - Set timeout parameters for context
 * @fctx:		Fabrics context
 * @keep_alive_tmo:	Keep-alive timeout in seconds (0 = kernel default)
 * @reconnect_delay:	Reconnect delay in seconds (0 = kernel default)
 * @ctrl_loss_tmo:	Controller loss timeout in seconds
 * @fast_io_fail_tmo:	Fast I/O fail timeout in seconds (0 = disabled)
 *
 * Sets timeout parameters for the context.
 *
 * Return: 0 on success, or a negative error code on failure.
 */
int libnvmf_context_set_timeout_options(struct libnvmf_context *fctx,
		int keep_alive_tmo, int reconnect_delay,
		int ctrl_loss_tmo, int fast_io_fail_tmo);

/**
 * libnvmf_context_set_connect_flags() - Set connection flags for context
 * @fctx:		Fabrics context
 * @tos:		Type of service (-1 = not set)
 * @tls_configured_key:	Pre-configured TLS PSK numeric key ID (0 = not set)
 * @duplicate_connect:	Allow duplicate connections to the same target
 * @disable_sqflow:	Disable controller SQ flow control
 * @hdr_digest:		Enable header digest (TCP)
 * @data_digest:	Enable data digest (TCP)
 * @tls:		Enable TLS (TCP)
 * @concat:		Enable secure concatenation (TCP)
 *
 * Sets connection flags and optional features for the context.
 *
 * Return: 0 on success, or a negative error code on failure.
 */
int libnvmf_context_set_connect_flags(struct libnvmf_context *fctx,
