...
%extend libnvme_ctrl {
	...
	void connect(struct libnvme_host *h,
		...
		const char *dev;
		/*
		 * Converting code: extract cfg from context for libnvmf_add_ctrl(),
		 * which currently takes libnvme_fabrics_config * but will eventually
		 * be updated to accept libnvmf_context * directly.
		 */
