...
static int twsc_get_ctrl_name_from_dev(struct nvme_dev *dev,
		char *ctrl_name, size_t size)
{
	struct stat st;
	char dev_path[48] = {};
	char sys_path[64] = {};
	...
	if (S_ISCHR(st.st_mode)) {
		...
		snprintf(sys_path, sizeof(sys_path),
			"/sys/class/block/%s", dev->name);
		if (!realpath(sys_path, resolved)) {
			switch (errno) {
			case ENOENT:
				fprintf(stderr,
					"Device sysfs path %s does not exist\n",
					sys_path);
				break;
			case EACCES:
				fprintf(stderr,
...
static int twsc_get_pcie_slot_from_ctrl_name(const char *ctrl_name,
		char *pcie_slot, size_t size)
{
	char path[128] = {};
	int len;
...
static int twsc_read_sys_device(const char *pcie_slot, char *file_name,
...
static int twsc_device_valid_check(struct nvme_dev *dev,
			struct twsc_device_config *dev_cfg)
{
	...
	err = twsc_get_ctrl_name_from_dev(dev, dev_cfg->ctrl_name,
		sizeof(dev_cfg->ctrl_name));
	if (err)
		return err;

	err = twsc_get_pcie_slot_from_ctrl_name(dev_cfg->ctrl_name,
		dev_cfg->pcie_slot, sizeof(dev_cfg->pcie_slot));
	if (err)
		return err;

	err = twsc_read_sys_device(dev_cfg->pcie_slot, "vendor",
		dev_cfg->vendor_id, sizeof(dev_cfg->vendor_id));
	if (err)
		return err;

	err = twsc_read_sys_device(dev_cfg->pcie_slot, "device",
		dev_cfg->device_id, sizeof(dev_cfg->device_id));
	if (err)
		return err;

	for (i = 0; i < (sizeof(g_twsc_device_config) /
			sizeof(struct twsc_device_config)); i++) {
		if ((!strcmp(g_twsc_device_config[i].vendor_id,
				dev_cfg->vendor_id)) &&
			(!strcmp(g_twsc_device_config[i].device_id,
				dev_cfg->device_id))) {
			return 0;
		}
	}
...
static int twsc_read_cust_data(int fd, void  *data, __u32 data_len,
		__u32 custid, __u32 bufid, __u32 cdw11, __u32 cdw13,
		__u32 cdw14)
{
	int err = 0;
...
static int nvme_query_cap(int fd, __u32 nsid, __u32 data_len, void *data)
{
	...
	if (err) {
		fprintf(stderr,
			"Could not query freespace information (0xD6)\n");
	}
		...
		unsigned int nsid, const char *devname)
{
	...
	json_object_add_value_int(entry_stats,
		"normalized", smart->program_fail_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->program_fail_cnt.raw));
	json_object_add_value_object(dev_stats,
		"program_fail_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->erase_fail_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->erase_fail_cnt.raw));
	json_object_add_value_object(dev_stats,
		"erase_fail_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		 "normalized", smart->wear_leveling_cnt.norm);
	multi = json_create_object();
	json_object_add_value_int(multi,
		"min", le16_to_cpu(smart->wear_leveling_cnt.wear_level.min));
	json_object_add_value_int(multi,
		"max", le16_to_cpu(smart->wear_leveling_cnt.wear_level.max));
	json_object_add_value_int(multi,
		"avg", le16_to_cpu(smart->wear_leveling_cnt.wear_level.avg));
	json_object_add_value_object(entry_stats,
		"raw", multi);
	json_object_add_value_object(dev_stats,
		"wear_leveling", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->e2e_err_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->e2e_err_cnt.raw));
	json_object_add_value_object(dev_stats,
		"end_to_end_error_detection_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->crc_err_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->crc_err_cnt.raw));
	json_object_add_value_object(dev_stats,
		"crc_error_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->timed_workload_media_wear.norm);
	json_object_add_value_float(entry_stats,
		"raw", ((float)int48_to_long(smart->timed_workload_media_wear.raw)) / 1024);
	json_object_add_value_object(dev_stats,
		"timed_workload_media_wear", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->timed_workload_host_reads.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->timed_workload_host_reads.raw));
	json_object_add_value_object(dev_stats,
		"timed_workload_host_reads", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->timed_workload_timer.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->timed_workload_timer.raw));
	json_object_add_value_object(dev_stats,
		"timed_workload_timer", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->thermal_throttle_status.norm);
	multi = json_create_object();
	json_object_add_value_int(multi,
		"pct", smart->thermal_throttle_status.thermal_throttle.pct);
	json_object_add_value_int(multi,
		"cnt", smart->thermal_throttle_status.thermal_throttle.count);
	json_object_add_value_object(entry_stats,
		"raw", multi);
	json_object_add_value_object(dev_stats,
		"thermal_throttle_status", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->retry_buffer_overflow_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw",	  int48_to_long(smart->retry_buffer_overflow_cnt.raw));
	json_object_add_value_object(dev_stats,
		"retry_buffer_overflow_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->pll_lock_loss_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->pll_lock_loss_cnt.raw));
	json_object_add_value_object(dev_stats,
		"pll_lock_loss_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->nand_bytes_written.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->nand_bytes_written.raw));
	json_object_add_value_object(dev_stats,
		"nand_bytes_written", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->host_bytes_written.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->host_bytes_written.raw));
	json_object_add_value_object(dev_stats,
		"host_bytes_written", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->raid_recover_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->raid_recover_cnt.raw));
	json_object_add_value_object(dev_stats,
		"raid_recover_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->prog_timeout_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->prog_timeout_cnt.raw));
	json_object_add_value_object(dev_stats,
		"prog_timeout_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->erase_timeout_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->erase_timeout_cnt.raw));
	json_object_add_value_object(dev_stats,
		"erase_timeout_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->read_timeout_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->read_timeout_cnt.raw));
	json_object_add_value_object(dev_stats,
		"read_timeout_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->read_ecc_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->read_ecc_cnt.raw));
	json_object_add_value_object(dev_stats,
		"read_ecc_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->non_media_crc_err_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->non_media_crc_err_cnt.raw));
	json_object_add_value_object(dev_stats,
		"non_media_crc_err_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->compression_path_err_cnt.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->compression_path_err_cnt.raw));
	json_object_add_value_object(dev_stats,
		"compression_path_err_cnt", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->out_of_space_flag.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->out_of_space_flag.raw));
	json_object_add_value_object(dev_stats,
		"out_of_space_flag", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->physical_usage_ratio.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->physical_usage_ratio.raw));
	json_object_add_value_object(dev_stats,
		"physical_usage_ratio", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->grown_bb_count.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->grown_bb_count.raw));
	json_object_add_value_object(dev_stats,
		"grown_bb_count", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->system_area_life_remaining.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->system_area_life_remaining.raw));
	json_object_add_value_object(dev_stats,
		"system_area_life_remaining", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->user_available_space_rate.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->user_available_space_rate.raw));
	json_object_add_value_object(dev_stats,
		"user_available_space_rate", entry_stats);

	entry_stats = json_create_object();
	json_object_add_value_int(entry_stats,
		"normalized", smart->over_provisioning_rate.norm);
	json_object_add_value_int(entry_stats,
		"raw", int48_to_long(smart->over_provisioning_rate.raw));
	json_object_add_value_object(dev_stats,
		"over_provisioning_rate", entry_stats);
		...
		unsigned int nsid, const char *devname)
{
	...
	printf("grown_bb_count                  : %3d%%       %" PRIu64 "\n",
		smart->grown_bb_count.norm,
		int48_to_long(smart->grown_bb_count.raw));
	...
	printf("user_available_space_rate       : %3d%%       %" PRIu64 "\n",
		smart->user_available_space_rate.norm,
		int48_to_long(smart->user_available_space_rate.raw));
	printf("over_provisioning_rate          : %3d%%       %" PRIu64 "\n",
		smart->over_provisioning_rate.norm,
		int48_to_long(smart->over_provisioning_rate.raw));
}

static int twsc_get_additional_smart_log(int argc, char **argv,
		struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_nsid_log(dev_fd(dev), false, TWSC_LOG_ADDL_SMART,
		cfg.namespace_id, sizeof(smart_log), (void *)&smart_log);
	if (err)
		goto close_dev;

	if (cfg.json)
		show_twsc_smart_log_json(&smart_log,
			cfg.namespace_id, dev->name);
	else if (!cfg.raw_binary)
		show_twsc_smart_log(&smart_log,
			cfg.namespace_id, dev->name);
	else
...
static int twsc_query_cap_info(int argc, char **argv, struct command *cmd,
...
static int nvme_dump_evtlog(struct nvme_dev *dev,
		struct twsc_device_config *dev_cfg,
		__u32 storage_medium, char *file)
{
	...
	if (err) {
		fprintf(stderr,
			"Unable to get evtlog lsp=0x%x, ret = 0x%x\n",
			args.lsp, err);
		goto ret;
	}
	...
	while (length > 0) {
		...
		if (err) {
			fprintf(stderr,
				"Unable to get evtlog offset=0x%llx len 0x%x ret = 0x%x\n",
				offset, args.len, err);
			goto close_fd;
		}
...
static int twsc_dump_evtlog(int argc, char **argv, struct command *cmd,
		struct plugin *plugin)
{
...
static int twsc_exit_write_reject(int argc, char **argv, struct command *cmd,
		struct plugin *plugin)
{
	struct nvme_dev *dev;
	struct twsc_device_config dev_cfg;
	...
	err = twsc_read_cust_data(dev_fd(dev), &status,
		sizeof(status), 0x89, 0, 0, 0, 0);
	if (err)
		goto close_dev;
...
static int twsc_get_pcie_status(const char *pcie_slot, char *pcie_status,
		size_t size)
{
	...
	snprintf(pcie_status, size, "%s",
		(pcie_uncorr_err != 0 ||
		 pcie_fatal_err != 0 ||
		 pcie_other_err != 0) ? "Warning":"Good");

	pclose(fp);
	return 0;
}

static int twsc_status(int argc, char **argv, struct command *cmd,
		struct plugin *plugin)
{
	...
	char pci_ssvid[7], link_speed[20], link_width[5], link_string[40];
	char buffer[512], numa_node[5], form_factor[15], temperature[10];
	char io_speed[15], serial_number[21], model_number[41];
	char firmware_revision[9], pcie_status[9];
	...
	err = twsc_read_sys_device(dev_cfg.pcie_slot, "subsystem_vendor",
		pci_ssvid, sizeof(pci_ssvid));
	if (err)
		goto close_dev;

	err = twsc_read_sys_device(dev_cfg.pcie_slot, "current_link_speed",
	...
	err = twsc_read_sys_device(dev_cfg.pcie_slot, "current_link_width",
		link_width, sizeof(link_width));
	if (err)
		goto close_dev;

	snprintf(link_string, 40, "Speed %s, Width x%s", link_speed, link_width);

	err = twsc_read_sys_device(dev_cfg.pcie_slot, "numa_node",
		numa_node, sizeof(numa_node));
	if (err)
		goto close_dev;

	err = twsc_get_pcie_status(dev_cfg.pcie_slot,
		pcie_status, sizeof(pcie_status));
	if (err)
		goto close_dev;
	...
	if (err) {
		fprintf(stderr,
			"Unable to read nvme_identify_ctrl() error code:%x\n",
			err);
		goto close_dev;
	}
	...
	snprintf(temperature, 10, "%li",
		kelvin_to_celsius(smart_log.temperature[1]<<8 |
			smart_log.temperature[0]));

	//Populate VU SMART log (Depends on device)
	err = nvme_get_log_simple(dev_fd(dev), TWSC_LOG_EXTENDED_HEALTH,
		sizeof(twsc_smart), (void *)&twsc_smart);
	if (err) {
	...
	case 'P':
		...
		break;
	case 'U':
		snprintf(form_factor, 15, "%s",
			(twsc_smart.opn[4] == '8')?"U.3":"U.2");
		break;
	case 'E':
	...
	err = nvme_get_nsid_log(dev_fd(dev), false, TWSC_LOG_ADDL_SMART,
		NVME_NSID_ALL, sizeof(struct nvme_additional_smart_log),
		(void *)&add_smart_log);
	if (err) {
	...
	write_amp = int48_to_long(add_smart_log.nand_bytes_written.raw)/
		(1.0 * int48_to_long(add_smart_log.host_bytes_written.raw));

	//Get TWSC freespace information
	err = nvme_query_cap(dev_fd(dev), NVME_NSID_ALL,
		sizeof(freespace_ctx), &freespace_ctx);
	if (err)
		goto close_dev;
	...
	if (twsc_smart.twsc_critical_warning & TWSC_CRIT_OVER_CAP) {
		err = twsc_read_cust_data(dev_fd(dev), &oos_extended_capacity,
			sizeof(oos_extended_capacity), 0x88, 0, 0, 0, 0);
		if (err)
			goto close_dev;
	...
	if (cfg.json) {
		...
		json_object_add_value_string(dev_stats,
		...
		json_object_add_value_uint(dev_stats,
			"Power Consumption [mW]",
			twsc_smart.power_mw_consumption);
		json_object_add_value_string(dev_stats,
			"Atomic Write Mode", "On");
		json_object_add_value_int(dev_stats,
			"Percentage Used", smart_log.percent_used);
		json_object_add_value_string(dev_stats,
			"Data Read",
			uint128_t_to_si_string(le128_to_cpu(smart_log.data_units_read), 1000 * 512));
		json_object_add_value_string(dev_stats,
			"Data Written",
			uint128_t_to_si_string(le128_to_cpu(smart_log.data_units_written), 1000 * 512));
		json_object_add_value_int(dev_stats,
			"Correctable Error Count",
			twsc_smart.pcie_rx_correct_errs);
		json_object_add_value_int(dev_stats,
			"Uncorrectable Error Count",
			twsc_smart.pcie_rx_uncorrect_errs);
		json_object_add_value_string(link_stats,
			"PCIe Link Width", link_width);
		json_object_add_value_string(link_stats,
			"PCIe Link Speed", link_speed);
		json_object_add_value_string(link_stats,
			"PCIe Device Status", pcie_status);
		json_object_add_value_object(dev_stats,
			"PCIe Link Status",	link_stats);
		if (twsc_smart.friendly_changecap_support) {
			json_object_add_value_int(dev_stats,
				"Current Formatted Capacity [GB]",
				twsc_smart.cur_formatted_capability);
			ijson_object_add_value_int(dev_stats,
				"Max Formatted Capacity [GB]",
				twsc_smart.max_formatted_capability);
			json_object_add_value_int(dev_stats,
				"Extendible Capacity LBA count",
				twsc_smart.extendible_cap_lbacount);
		} else {
			json_object_add_value_int(dev_stats,
		...
		json_object_add_value_int(dev_stats,
			...
			IDEMA_CAP2GB(twsc_smart.total_physical_capability));
		json_object_add_value_int(dev_stats,
			"Compression Ratio",
			twsc_smart.comp_ratio);
		json_object_add_value_int(dev_stats,
			"Physical Used Ratio",
			twsc_smart.physical_usage_ratio);
		json_object_add_value_int(dev_stats,
			"Free Physical Space [GB]",
			IDEMA_CAP2GB(twsc_smart.free_physical_capability));
		json_object_add_value_string(dev_stats,
			"RSA Verify",
			(twsc_smart.otp_rsa_en) ? "ON" : "OFF");
		json_object_add_value_string(dev_stats,
			"IO Speed",	io_speed);
		if (twsc_smart.twsc_critical_warning & TWSC_CRIT_OVER_CAP) {
			json_object_add_value_int(dev_stats,
				"OOS Extended Capacity [GiB]",
				oos_extended_capacity);
		}
		json_object_add_value_string(dev_stats,
			"NUMA Node", numa_node);
		json_object_add_value_int(dev_stats,
			"Indirection Unit [kiB]",
			(4*freespace_ctx.map_unit));
		json_object_add_value_double(dev_stats,
			"Lifetime WAF", write_amp);
		json_object_add_value_int(crit_stats,
			"Thermal Throttling On",
			(twsc_smart.temp_throttle_info));
		json_object_add_value_int(crit_stats,
			"Backup Capacitor Status Bad",
			(smart_log.critical_warning & 0x10));
		json_object_add_value_int(crit_stats,
			"Bad block exceeds threshold",
			(smart_log.critical_warning & 0x01));
		json_object_add_value_int(crit_stats,
			"Media Error",
		...
		json_object_add_value_int(crit_stats,
			...
			(twsc_smart.twsc_critical_warning & TWSC_CRIT_PWR_FAIL_DATA_LOSS));
		json_object_add_value_int(crit_stats,
			"Exceed physical capacity limitation",
			(twsc_smart.twsc_critical_warning & TWSC_CRIT_OVER_CAP));
		json_object_add_value_int(crit_stats,
			"Read/Write lock mode",
			(twsc_smart.twsc_critical_warning & TWSC_CRIT_RW_LOCK));

		json_object_add_value_object(dev_stats,
			"Critical Warning(s)", crit_stats);

		json_object_add_value_object(root,
			"Device stats", dev_stats);
		...
		printf("%-35s%s\n",    "Data Written:",
			uint128_t_to_si_string(le128_to_cpu(smart_log.data_units_written), 1000 * 512));
		printf("%-35s%i\n",    "Correctable Error Cnt:",
			add_smart_log.twsc_smart.pcie_rx_correct_errs);
		printf("%-35s%i\n",    "Uncorrectable Error Cnt:",
			twsc_smart.pcie_rx_uncorrect_errs);
		...
		if (twsc_smart.friendly_changecap_support) {
			printf("%-35s%"PRIu64" GB\n",
				...
				(uint64_t)twsc_smart.cur_formatted_capability);
			printf("%-35s%"PRIu64" GB\n",
				"Max Formatted Capacity:",
				(uint64_t)twsc_smart.max_formatted_capability);
			printf("%-35s%"PRIu64"\n",
				"Extendible Capacity LBA count:",
				(uint64_t)twsc_smart.extendible_cap_lbacount);
		} else {
			printf("%-35s%"PRIu64" GB\n", "Formatted  Capacity:",
				(uint64_t)twsc_smart.max_formatted_capability);
		}
		printf("%-35s%"PRIu64" GB\n", "Provisioned Capacity:",
			(uint64_t)IDEMA_CAP2GB(twsc_smart.total_physical_capability));
		printf("%-35s%u%%\n",         "Compression Ratio:",
			twsc_smart.comp_ratio);
		printf("%-35s%u%%\n",         "Physical Used Ratio:",
			twsc_smart.physical_usage_ratio);
		printf("%-35s%"PRIu64" GB\n", "Free Physical Space:",
			(uint64_t)IDEMA_CAP2GB(twsc_smart.free_physical_capability));
		printf("%-35s%s\n",           "RSA Verify:",
				"(twsc_smart.otp_rsa_en) ? "ON":"OFF");
		printf("%-35s%s\n",           "IO Speed:",
			io_speed);
		if (twsc_smart.twsc_critical_warning & TWSC_CRIT_OVER_CAP) {
			printf("%-35s%"PRIu64" GiB\n", "OOS Extended Capacity:",
				(uint64_t)oos_extended_capacity);
		}
		printf("%-35s%s\n",           "NUMA Node:",
			numa_node);
		printf("%-35s%"PRIu64"K\n",   "Indirection Unit:",
			(uint64_t)(4*freespace_ctx.map_unit));
		printf("%-35s%.2f\n",	      "Lifetime WAF:",
			write_amp);
		printf("%-35s%s\n",           "Critical Warning(s):",
			buffer);
