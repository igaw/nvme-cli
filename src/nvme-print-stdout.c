...
static struct shr_table *stdout_persistent_event_entry_ehai_table(__u8 ehai)
{
	...
	stdout_bits_add(t, "[1:0]", pit, "%s", nvme_pel_ehai_pit_to_string(pit));
...
void nvme_show_pel_header(struct nvme_persistent_event_log *pevent_log_head, int verbose)
{
	struct shr_table *t;
	int row;
	...
	stdout_kv_add(t, "Log Identifier", "%u", pevent_log_head->lid);
	...
	stdout_kv_add(t, "Log Revision", "%u", pevent_log_head->rv);
	stdout_kv_add(t, "Log Header Length", "%u", pevent_log_head->lhl);
	stdout_kv_add(t, "Timestamp", "%"PRIu64, le64_to_cpu(pevent_log_head->ts));
				...
				int verbose)
{
	...
	stdout_kv_add(t, "Event Type Revision", "%u", pevent_entry_head->etype_rev);
	stdout_kv_add(t, "Event Header Length", "%u", pevent_entry_head->ehl);
	row = stdout_kv_add(t, "Event Header Additional Info", "%u",
			     pevent_entry_head->ehai);
	if (verbose)
		shr_table_set_row_subtable(t, row,
			stdout_persistent_event_entry_ehai_table(pevent_entry_head->ehai));
...
void nvme_show_pel_fw_commit_event(void *pevent_log_info, __u32 offset)
{
	...
	stdout_kv_add(t, "FW Commit Action", "%u", fw_commit_event->fw_commit_action);
					...
					struct nvme_persistent_event_entry *pevent_entry_head)
{
	...
	for (int i = 0; i < por_info_list; i++) {
		...
		stdout_kv_add(t, "Controller ID", "%u", le16_to_cpu(por_event->cid));
		stdout_kv_add(t, "Firmware Activation", "%u", por_event->fw_act);
		stdout_kv_add(t, "Operation in Progress", "%u", por_event->op_in_prog);
...
void nvme_show_pel_nss_hw_error_event(void *pevent_log_info, __u32 offset)
{
	...
	stdout_kv_add(t, "NVM Subsystem Hardware Error Event Code Entry", "%u, %s",
...
void nvme_show_pel_change_ns_event(void *pevent_log_info, __u32 offset)
{
	...
	stdout_kv_add(t, "Namespace Size", "%"PRIu64, le64_to_cpu(ns_event->nsze));
	stdout_kv_add(t, "Namespace Capacity", "%"PRIu64, le64_to_cpu(ns_event->nscap));
	stdout_kv_add(t, "Formatted LBA Size", "%u", ns_event->flbas);
	stdout_kv_add(t, "End-to-end Data Protection Type Settings", "%u",
		      ns_event->dps);
	stdout_kv_add(t, "Namespace Multi-path I/O and Namespace Sharing Capabilities",
		      "%u", ns_event->nmic);
	stdout_kv_add(t, "ANA Group Identifier", "%u", le32_to_cpu(ns_event->ana_grp_id));
...
void nvme_show_pel_format_start_event(void *pevent_log_info, __u32 offset)
{
	...
	stdout_kv_add(t, "Namespace Identifier", "%u", le32_to_cpu(format_start_event->nsid));
	stdout_kv_add(t, "Format NVM Attributes", "%u", format_start_event->fna);
...
void nvme_show_pel_format_completion_event(void *pevent_log_info, __u32 offset)
{
	...
	stdout_kv_add(t, "Format NVM Status", "%u", format_cmpln_event->format_nvm_status);
					 ...
					 __u32 event_data_len)
{
	...
	for (i = 0; progress < event_data_len; i++) {
		...
		stdout_kv_add(t, "Vendor Specific Event Data Type", "%u", vs_desc->vsedt);
...
static struct shr_table *stdout_fid_support_effects_log_verbose_table(__u32 fid_support)
{
	struct shr_table *t;
	...
	stdout_bits_add(t, "[20:20]", ns_scope, "Namespace Scope %sIndicated",
			ns_scope ? "" : "Not ");
	stdout_bits_add(t, "[21:21]", ctrl_scope, "Controller Scope %sIndicated",
			ctrl_scope ? "" : "Not ");
					   ...
					   const char *devname)
{
	...
	for (i = 0; i < 256; i++) {
		...
		if (verbose)
			shr_table_set_row_subtable(t, row,
				stdout_fid_support_effects_log_verbose_table(fid_effect));
	}

	stdout_kv_table_finish(t, "fid-support-effects-log");
}

static struct shr_table *stdout_mi_cmd_support_effects_log_verbose_table(__u32 mi_cmd_support)
{
	struct shr_table *t;
	...
	__u8 nvmset_scope = !!(csp & NVME_MI_CMD_SUPPORTED_EFFECTS_SCOPE_NVM_SET);
	__u8 endgrp_scope = !!(csp & NVME_MI_CMD_SUPPORTED_EFFECTS_SCOPE_ENDGRP);
	__u8 domain_scope = !!(csp & NVME_MI_CMD_SUPPORTED_EFFECTS_SCOPE_DOMAIN);
					      ...
					      const char *devname)
{
	...
	for (i = 0; i < NVME_LOG_MI_CMD_SUPPORTED_EFFECTS_MAX; i++) {
		...
		if (verbose)
			shr_table_set_row_subtable(t, row,
				stdout_mi_cmd_support_effects_log_verbose_table(mi_cmd_effect));
	}
...
static void stdout_phy_rx_eom_descs(struct nvme_phy_rx_eom_log *log, size_t len)
{
	...
	while ((desc = eom_desc_iter_next(&it))) {
		...
		stdout_kv_add(t, "Number of Rows", "%u", le16_to_cpu(desc->nrows));
...
static void stdout_ctrl_register_common(int offset, uint64_t value, bool fabrics)
{
	...
	if (verbose) {
		...
		shr_table_set_row_subtable(t, row,
				stdout_ctrl_register_verbose_table(offset, value,
								  true));
	} else {
...
static struct shr_table *stdout_id_ctrl_ctratt_table(__le32 ctrl_ctratt)
{
	...
	stdout_bits_add(t, "[20:20]", pls, "Power Limit %sSupported",
			 pls ? "" : "Not ");
	stdout_bits_add(t, "[19:19]", fdps, "Flexible Data Placement %sSupported",
			 fdps ? "" : "Not ");
	...
	stdout_bits_add(t, "[12:12]", vcm,
			 ...
			 vcm ? "" : "Not ");
	stdout_bits_add(t, "[11:11]", fcm, "Fixed Capacity Management %sSupported",
			 fcm ? "" : "Not ");
	...
	stdout_bits_add(t, "[7:7]", ng, "Namespace Granularity %sSupported",
			 ng ? "" : "Not ");
	stdout_bits_add(t, "[6:6]", tbkas, "Traffic Based Keep Alive %sSupported",
			 tbkas ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_chsi_table(__u8 ctrl_chsi)
{
	...
	stdout_bits_add(t, "[0:0]", chs, "CXL HDM %sSupported", chs ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_nvmsr_table(__u8 nvmsr)
{
	...
	if (rsvd)
		stdout_bits_add(t, "[7:2]", rsvd, "Reserved");
	stdout_bits_add(t, "[1:1]", nvmee, "NVM subsystem %spart of an Enclosure",
			 nvmee ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_oacs_table(__le16 ctrl_oacs)
{
	...
	stdout_bits_add(t, "[8:8]", dbc, "Doorbell Buffer Config %sSupported",
			 dbc ? "" : "Not ");
	stdout_bits_add(t, "[7:7]", vir, "Virtualization Management %sSupported",
			 vir ? "" : "Not ");
	...
	stdout_bits_add(t, "[1:1]", fmt, "Format NVM %sSupported",
			 fmt ? "" : "Not ");
	stdout_bits_add(t, "[0:0]", sec, "Security Send and Receive %sSupported",
			 sec ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_lpa_table(__u8 lpa)
{
	...
	stdout_bits_add(t, "[3:3]", telem,
			 ...
			 telem ? "" : "Not ");
	stdout_bits_add(t, "[2:2]", ed, "Extended data for Get Log Page %sSupported",
			 ed ? "" : "Not ");
	stdout_bits_add(t, "[1:1]", celp, "Command Effects Log Page %sSupported",
			 celp ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_sanicap_table(__le32 ctrl_sanicap)
{
	...
	stdout_bits_add(t, "[3:3]", vers,
			 ...
			 vers ? "" : "Not ");
	stdout_bits_add(t, "[2:2]", ows, "Overwrite Sanitize Operation %sSupported",
			 ows ? "" : "Not ");
	stdout_bits_add(t, "[1:1]", bes, "Block Erase Sanitize Operation %sSupported",
			 bes ? "" : "Not ");
	stdout_bits_add(t, "[0:0]", ces, "Crypto Erase Sanitize Operation %sSupported",
			 ces ? "" : "Not ");
...
static struct shr_table *stdout_id_ctrl_vsen_table(__le32 ctrl_vsen)
{
	...
	if (!vsen) {
		...
		shr_table_set_value_str(t, 3, row, "Voltage sensor not supported", LEFT);
		shr_table_add_row(t, row);
...
static struct shr_table *stdout_id_ctrl_sqes_table(__u8 sqes)
{
	...
	stdout_bits_add(t, "[7:4]", msqes, "Max SQ Entry Size (%d)", 1 << msqes);
	stdout_bits_add(t, "[3:0]", rsqes, "Min SQ Entry Size (%d)", 1 << rsqes);
...
static struct shr_table *stdout_id_ctrl_cqes_table(__u8 cqes)
{
	...
	stdout_bits_add(t, "[7:4]", mcqes, "Max CQ Entry Size (%d)", 1 << mcqes);
	stdout_bits_add(t, "[3:0]", rcqes, "Min CQ Entry Size (%d)", 1 << rcqes);
...
static struct shr_table *stdout_id_ctrl_oncs_table(__le16 ctrl_oncs)
{
	...
	stdout_bits_add(t, "[2:2]", nvmdsmsv, "Dataset Management Support Variants");
	stdout_bits_add(t, "[1:1]", nvmwusv, "Write Uncorrectable Support Variants");
...
static struct shr_table *stdout_id_ctrl_sgls_table(__le32 ctrl_sgls)
{
	...
	if (sglsp == 0x3)
		...
		stdout_bits_add(t, "[1:0]", sglsp, "Scatter-Gather Lists Not Supported");
...
static struct shr_table *stdout_id_ctrl_trattr_table(__u8 ctrl_trattr)
{
	...
	if (rsvd3)
		stdout_bits_add(t, "[7:3]", rsvd3, "Reserved");
	stdout_bits_add(t, "[2:2]", mrtll, "Memory Range Tracking Length Limit");
...
static char *stdout_power_and_scale_str(__u16 power, __u8 scale)
{
	...
	case NVME_PSD_PS_100_MICRO_WATT:
		if (asprintf(&s, "%01u.%04uW", power / 10000, power % 10000) < 0)
			s = NULL;
		break;
...
static char *stdout_psd_time_str(__u8 time, __u8 ts)
{
	static const struct {
		...
		{ 1, "ms" }, { 10, "ms" }, { 100, "ms" },
		{ 1, "s" }, { 10, "s" }, { 100, "s" },
		{ 1000, "s" }, { 10000, "s" }, { 100000, "s" }, { 1000000, "s" },
	};
	char *s = NULL;
	...
	case 1 ... 99:
		if (ts >= ARRAY_SIZE(scale)) {
			if (asprintf(&s, "reserved") < 0)
				s = NULL;
		} else if (asprintf(&s, "%u%s", time * scale[ts].mult, scale[ts].unit) < 0) {
			s = NULL;
		}
		break;
	default:
...
static struct shr_table *stdout_id_ctrl_ps_table(struct nvme_id_ctrl *ctrl)
{
	...
	for (i = 0; i <= ctrl->npss; i++) {
		...
		if (psd->flags & NVME_PSD_FLAGS_MXPS) {
			if (asprintf(&mp, "%01u.%04uW", max_power / 10000, max_power % 10000) < 0)
				mp = NULL;
		} else {
			if (asprintf(&mp, "%01u.%02uW", max_power / 100, max_power % 100) < 0)
				mp = NULL;
		}

		idle_power = stdout_power_and_scale_str(le16_to_cpu(psd->idlp),
							 nvme_psd_power_scale(psd->ips));
		active_power = stdout_power_and_scale_str(le16_to_cpu(psd->actp),
		...
		if (iiellss) {
			...
			if (miiell_val) {
				if (asprintf(&miiell, "%uus", miiell_val * 100) < 0)
					miiell = NULL;
			} else {
...
{
	bool verbose = stdout_print_ops.flags & VERBOSE, vs = stdout_print_ops.flags & VS;
	struct shr_table *t;
	int row;
	...
	if (NVME_CTRL_CTRATT_VMS(le32_to_cpu(ctrl->ctratt))) {
		row = stdout_kv_add(t, "vsen1", "%#x", le32_to_cpu(ctrl->vsen1));
		if (verbose)
			shr_table_set_row_subtable(t, row,
					stdout_id_ctrl_vsen_table(ctrl->vsen1));

		row = stdout_kv_add(t, "vsen2", "%#x", le32_to_cpu(ctrl->vsen2));
		if (verbose)
			shr_table_set_row_subtable(t, row,
					stdout_id_ctrl_vsen_table(ctrl->vsen2));

		row = stdout_kv_add(t, "vsen3", "%#x", le32_to_cpu(ctrl->vsen3));
		if (verbose)
			shr_table_set_row_subtable(t, row,
					stdout_id_ctrl_vsen_table(ctrl->vsen3));

		row = stdout_kv_add(t, "vsen4", "%#x", le32_to_cpu(ctrl->vsen4));
		if (verbose)
			shr_table_set_row_subtable(t, row,
	...
	stdout_kv_add(t, "subnqn", "%-.*s", (int)sizeof(ctrl->subnqn), ctrl->subnqn);
...
static void stdout_id_domain_list(struct nvme_id_domain_list *id_dom)
{
	...
	for (i = 0; i < id_dom->num; i++) {
		...
		snprintf(name, sizeof(name), "Domain Capacity for Attr Entry[%u]", i);
		...
		snprintf(name, sizeof(name),
			 "Max Endurance Group Domain Capacity for Attr Entry[%u]", i);
		stdout_kv_add(t, name, "%s", uint128_t_to_l10n_string(
				    ...
				    struct nvme_cmd_effects_log *effects)
{
	...
	stdout_effects_log_segment(1, 0, 0xbf, effects, "Admin Commands", verbose);
	stdout_effects_log_segment(1, 0xc0, 0xff, effects, "Vendor Specific Admin Commands", verbose);
	stdout_effects_log_segment(0, 0, 0x80, effects, "I/O Commands", verbose);
	stdout_effects_log_segment(0, 0x80, 0x100, effects, "Vendor Specific I/O Commands", verbose);
}
...
static struct shr_table *stdout_support_log_verbose_table(__u32 support, __u8 lid)
{
	struct shr_table *t;
...
static void stdout_auto_pst(struct nvme_feat_auto_pst *apst)
{
	...
	for (i = 0; i < ARRAY_SIZE(apst->apst_entry); i++) {
		...
		shr_table_set_indent(t, 2);
		stdout_kv_add(t, "Idle Time Prior to Transition (ITPT)", "%u ms",
	...
	case NVME_DIRECTIVE_DTYPE_STREAMS:
		...
		case NVME_DIRECTIVE_RECEIVE_STREAMS_DOPER_PARAM:
			...
			stdout_kv_add(t, "Max Streams Limit (MSL)", "%u",
				      *(__u16 *)field);
			stdout_kv_add(t, "NVM Subsystem Streams Available (NSSA)",
				      "%u", *(__u16 *)(field + 2));
			stdout_kv_add(t, "NVM Subsystem Streams Open (NSSO)", "%u",
				      *(__u16 *)(field + 4));
			stdout_kv_add(t, "NVM Subsystem Stream Capability (NSSC)",
				      "%u", *(__u16 *)(field + 6));
			stdout_kv_add(t, "Stream Write Size (in unit of LB size) (SWS)",
				      "%u", *(__u32 *)(field + 16));
			stdout_kv_add(t, "Stream Granularity Size (in unit of SWS) (SGS)",
				      "%u", *(__u16 *)(field + 20));
		...
		case NVME_DIRECTIVE_RECEIVE_STREAMS_DOPER_STATUS:
			...
			stdout_kv_add(t, "Open Stream Count", "%u", *(__u16 *)field);
			for (i = 0; i < count; i++) {
				char name[32];

				snprintf(name, sizeof(name), "Stream Identifier %.6u",
					 i + 1);
				stdout_kv_add(t, name, "%u",
					      *(__u16 *)(field + ((i + 1) * 2)));
			}
...
static void stdout_feat_perfc_id_list(struct nvme_perf_attr_id_list *data)
{
	...
	stdout_kv_add(t, "maximum saveable vendor specific performance attributes (MSVSPA)",
		      "%d", data->msvspa);
				       ...
				       unsigned char *buf)
{
	...
	case NVME_FEAT_FID_POWER_MGMT:
		...
		stdout_kv_add(t, "Power State (PS)", "%u", NVME_FEAT_PM_PS(result));
	...
	case NVME_FEAT_FID_TEMP_THRESH:
		...
		stdout_kv_add(t, "Temperature Threshold (TMPTH)",
			      "%s (%u K, %s)",
			      nvme_degrees_string(NVME_FEAT_TT_TMPTH(result)),
	...
	case NVME_FEAT_FID_ERR_RECOVERY:
		...
		stdout_kv_add(t,
			      ...
			      "%s", NVME_FEAT_ER_DULBE(result) ? "Enabled" : "Disabled");
	...
	case NVME_FEAT_FID_NUM_QUEUES:
		...
		stdout_kv_add(t, "Number of IO Completion Queues Allocated (NCQA)",
	...
	case NVME_FEAT_FID_HOST_MEM_BUF:
		...
		stdout_kv_add(t, "Host Memory Non-operational Access Restriction Enable (HMNARE)",
	...
	case NVME_FEAT_FID_HCTM:
		...
		stdout_kv_add(t, "Thermal Management Temperature 1 (TMT1)",
			      "%u K (%s, %s)", NVME_FEAT_HCTM_TMT1(result),
		...
		stdout_kv_add(t, "Thermal Management Temperature 2 (TMT2)",
			      "%u K (%s, %s)", NVME_FEAT_HCTM_TMT2(result),
	...
	case NVME_FEAT_FID_HOST_BEHAVIOR:
		if (buf) {
			...
			stdout_kv_add(t, "Advanced Command Retry Enable (ACRE)",
				      "%s", host_behavior->acre ? "True" : "False");
			stdout_kv_add(t, "Extended Telemetry Data Area 4 Supported (ETDAS)",
				      "%s", host_behavior->etdas ? "True" : "False");
			stdout_kv_add(t, "LBA Format Extension Enable (LBAFEE)",
				      "%s", host_behavior->lbafee ? "True" : "False");
			stdout_kv_add(t, "Host Dispersed Namespace Support (HDISNS)",
				      "%s", host_behavior->hdisns ? "Enabled" : "Disabled");
			stdout_kv_add(t, "Copy Descriptor Format 2h Enabled (CDF2E)",
				      "%s", host_behavior->cdfe & (1 << 2) ? "True" : "False");
			stdout_kv_add(t, "Copy Descriptor Format 3h Enabled (CDF3E)",
				      "%s", host_behavior->cdfe & (1 << 3) ? "True" : "False");
	...
	case NVME_FEAT_FID_RESV_NF_MASK:
		...
		stdout_kv_add(t, "Mask Reservation Preempted Notification (RESPRE)",
			      "%s", NVME_FEAT_RM_RESPRE(result) ? "True" : "False");
		stdout_kv_add(t, "Mask Reservation Released Notification (RESREL)",
			      "%s", NVME_FEAT_RM_RESREL(result) ? "True" : "False");
	...
	case NVME_FEAT_FID_BP_WRITE_PROTECT:
		...
		stdout_kv_add(t, "Boot Partition 1 Write Protection State (BP1WPS)",
	...
	case NVME_FEAT_FID_POWER_LIMIT: {
		...
		power_str = stdout_power_and_scale_str(NVME_FEAT_POWER_LIMIT_PLV(result),
							field);
	...
	case NVME_FEAT_FID_POWER_THRESH: {
		...
		power_str = stdout_power_and_scale_str(NVME_FEAT_POWER_THRESH_PTV(result),
							field);
		stdout_kv_add(t, "Power Threshold Scale (PTS)", "%u - %s",
			      field, nvme_feature_power_limit_scale_to_string(field));
				  ...
				  int numrec)
{
	...
	for (i = 0; i < numrec; i++) {
		...
		stdout_kv_add(t, "adrfam", "%s",
			      e->traddr[0] ? libnvmf_adrfam_str(e->adrfam) : "");
		stdout_kv_add(t, "subtype", "%s", libnvmf_subtype_str(e->subtype));
...
static void stdout_host_discovery_log(struct nvme_host_discovery_log *log)
{
	...
	for (i = sizeof(*log); i < le32_to_cpu(log->thdlpl); i += tel) {
		...
		stdout_kv_add(t, "trtype", "%s", libnvmf_trtype_str(hedlpe->trtype));
		stdout_kv_add(t, "adrfam", "%s",
