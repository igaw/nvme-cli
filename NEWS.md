...
* The passthru command functions have been renamed to clarify their
  semantics. The old `nvme_submit_admin_passthru()` /
  `nvme_submit_io_passthru()` names are gone. Synchronous execution
  is now `libnvme_exec_admin_passthru()` /
  `libnvme_exec_io_passthru()`; the new explicit async path uses
  `libnvme_submit_admin_passthru()` /
  `libnvme_submit_io_passthru()` with `libnvme_wait_passthru()` /
  `libnvme_reap_passthru()` to collect results.
...
* `libnvme_refresh_topology()` now returns `int` instead of `void`.
  A return value of 0 means success; negative means failure. Callers
